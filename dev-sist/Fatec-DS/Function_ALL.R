...
configs <- function() {
  ...
  dir_desp_ate2024 <- paste0(
    diretorio,
    "/Orçamento_Publico/Despesas-LN-2008-atual.xlsx"
  )
  dir_rec_ate2024 <- paste0(
    diretorio,
    "/Orçamento_Publico/Receitas-LN-2008-atual.xlsx"
  )
  ...
  lista_mun_rmvale <- read_excel(
    path = dir_rmvale,
    sheet = "municipios_rmvale"
  ) # 39 municipios da RM_Vale
...
load_packages <- function() {
  ...
  if (sum(as.numeric(!pacotes %in% installed.packages())) != 0) {
    # retorna matriz pacotes ja instalados
...
download_receitas <- function(anos_f, lista_municipios_f) {
  ...
  tipo = "receitas"
  receitas_acum <- read.csv(
    file = "receitas_vazia.csv",
    sep = ";",
    header = T,
    encoding = "latin1"
  )
  ...
  for (ano in anos_f) {
    for (mun in lista_municipios_f) {
      ...
      url_baixar <- paste(
        "https://transparencia.tce.sp.gov.br/sites/default/files/csv/",
        tipo,
        "-",
        mun,
        "-",
...
download_despesas <- function(anos_f, lista_municipios_f) {
  ...
  tipo = "despesas"
  despesas_acum <- read.csv(
    file = "despesas_vazia.csv",
    sep = ";",
    header = T,
    encoding = "latin1"
  )
