...
transf_mun$valor <- transf_mun$`1º Decêndio` +
  transf_mun$`2º Decêndio` +
  transf_mun$`3º Decêndio`
# Sumarização por municipio

transf_mun_sum <- transf_mun %>%
  group_by(Município) %>% # , Transferência
  summarise(Valor_transf_ano = sum(valor))


tot <- transf_mun %>%
  group_by() %>% # , Transferência
  summarise(Valor_transf_ano = sum(valor))
