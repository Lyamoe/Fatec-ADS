...
anos <- c("2023") # "2021", "2022", "2023")    # colocar nesta variável, todos os anos que pretende baixar
...
####################### Procedimento para execução manual download despesas #######################################################################
{
    ...
    despesas_acum <- read.csv(
        file = "despesas-ilhabela-2024.csv",
        sep = ";",
        header = T,
        encoding = "latin1"
    )
    ...
    despesas_acum <- dplyr::filter(
        despesas_acum,
        tp_despesa == "Valor Liquidado"
    )
    ...
    despesas_acum$historico_std <- str_replace_all(
        despesas_acum$historico_std,
        "[^[:alnum:]]",
        ""
